...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 1. put(K key, V value) : 앱 공간에 key + value 세트 추가
		hm.put("불고기버거", new Burger("불고기맛", 138));
		hm.put("통새우와퍼", new Burger("살살녹는맛", 500));
		hm.put("빅맥", new Burger("살찌는맛", 800));
		hm.put("상하이치킨버거", new Burger("매콤치킨맛", 593));
		hm.put("핫칭즈징거버거", new Burger("매콤치킨맛", 593));
		hm.put("언빌리버블버거", new Burger("짭짤한맛", 1000));
		
		System.out.println(hm);
		// 값 저장 순서 유지 X
		// Value값의 중복 저장 허용
		
		hm.put("핫칭즈징거버거", new Burger("개맛", 0));
		
		System.out.println(hm);
		// 기존 key값으로 다른 Value를 추가한다면 덮어씌어짐!!
		// key는 식별자개념 !! => key로 value를 찾아간다.
		
		// 2. get(Object key) : Object
		// => 제네릭 설정해줬으므로 매개변수가 Sting, 반환형 Burger
		// 해당 key값에 해당하는 value를 반환해주는 메서드
		
		System.out.println(hm.get("핫칭즈징거버거"));
		
		Burger b = /*(Burger)*/hm.get("빅맥");
		// 제네릭 설정을 안해놨다면 매번 다운캐스팅을 해야한다.
		
		// 3. size() : 컬렉션에 담겨 있는 갯수
		System.out.println(hm.size());
		
		// 4. replace(K key, V value) : 해당 키 값을 찾아서 벨류를 변경시켜줌
		...
		// 해시맵에 들어있는 모든 요소들에 순차적으로 접근하고자 할때 ??
		// for 문 X, 향상된 for 문 X 
		// List에 addAll 하는방법..?
		// Iterator를 쓰는 방법 => 바로 호출은 안됌!
		
		// Map계열을 Set계열로 바꿔서 => Iterator를 사용할것!
		// 2가지 방법!
		
		// 1. keySet() 이용하는 방법
		// => HashMap에서 제공하는 메서드, Set에 Key들만 담아준다.
		// => 반환형은 Set
	...
	/*
	 * Map 계열 특성상 요소들에 순차적으로 접근할 직접적인 방법은 없다.!
	 * 
