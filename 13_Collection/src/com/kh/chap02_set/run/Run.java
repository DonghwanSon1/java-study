...
import com.kh.chap02_set.mode.vo.Student;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println(hs); // 저장 순서 유지 X , 중복저장 X
		...
		// 저장순서유지 X , 중복 저장 O
		// 왜 ??? => 동일 객체로 판단이 되지 않았기 때문에!
		// HashSet 특징 : 값이 추가 될때마다 equals()와 hashCode()로 비교후 둘다 결과가 true일 경우 동일 객체로 판단
		...
		/*
		 * 총 정리!
		 * HashSet에 객체를 담을 때 내부적으로 equals메서드랑
		 * hashCode메서드를 기준으로 값이 일치하는지를 비교하고 담는다.
		 ...
		 * 
		 * => 내용물은 같은데 주소값이 달라서 동일객체가 아닌걸로 판단이 되면서 중복저장이 되어버린다. => 방지하고 싶다면?
		 * equals 메서드랑 hashCode 메서드를 오버라이딩해서 사용해야한다.
		 * 
		 * Student 클래스
		 * 
		 * hashCode() : 세개의 필드값을 하나의 문자열로 만들고 해시코드값을 만들어서 반환
		 * equals() : 세개의 필드의 값이 모두 일치하면 true
		 * 
		 */
		...
		System.out.println(it.next());
		System.out.println(it.next());
		// System.out.println(it.next()); => NoSuchElement : 더 꺼낼거 없다!!
		
		while(it.hasNext())	{ // it에 next로 더 빼을값이 남아있나?? 있으면 true// 없으면 false
			System.out.println(it.next());
		}
