...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * - 모든 필드와 메서드가 다 static으로 되어있다.
		 * - 객체를 생성할 필요가 없으므로 생성자가 private이다.
		 */
		
		// 필드
		// 파이 => Math클래스 내에 상수필드로 정의되어있음
		// Math mt = new Math(); => private이라 안됀다.
		...
		// 8. 제곱 => Math.pow(doube, double) : double
		System.out.println("2의 5제곱 : " +  Math.pow(2, 5));
		
		
		
		/*
		 * 모든게 다 static이기 때문에 객체 생성할 필요가 없음 => Math클래스
		 *  => 싱글톤 패턴
		 *  		(프로그램 실행과 동시에 메모리영역에 올려노호 재사용하는 개념)
		 *  (객체 하나만으로 엉디에서든지 가져다 쓸 수 있는 개념 == 공유)
		 *  싱글톤패턴 개념은 JDBC
		 * 
		 */
	}
